...
impl AgentExecutor {
    ...
    pub async fn execute(&self, user_message: &str) -> Result<AgentResponse> {
        ...
        loop {
            ...
            let request = conversation.iter().fold(request, |req, msg| {
                req.add_message(msg.clone())
            });
    ...
    async fn execute_single_tool(&self, call: &ToolCall) -> Result<ToolResult> {
        ...
        if let Some(cache) = &self.cache {
            let cache_key = format!("{}:{}", call.name, serde_json::to_string(&call.params.raw).unwrap_or_default());
