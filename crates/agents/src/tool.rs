...
impl ToolParams {
    ...
    pub fn from_json(json: &str) -> Result<Self> {
        let value: Value = serde_json::from_str(json)
            .map_err(|e| crate::AgentError::ParseError(e.to_string()))?;
        Ok(Self { raw: value })
    }
    ...
    pub fn get_str(&self, key: &str) -> Result<String> {
        self.raw.get(key)
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
            .ok_or_else(|| crate::AgentError::InvalidParams(format!("Missing or invalid '{}'", key)))
    }
    
    /// Get optional string parameter
    pub fn get_str_opt(&self, key: &str) -> Option<String> {
        self.raw.get(key).and_then(|v| v.as_str()).map(|s| s.to_string())
    }
