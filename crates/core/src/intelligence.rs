...
impl PricingRegistry {
    ...
    pub fn calculate_cost(
        ...
        let tier = tiers.iter().find(|t| {
            t.provider == provider
                && (model == t.model_pattern
                    || model.starts_with(&t.model_pattern.replace("*", "")))
        });
            ...
            .and_modify(|s| {
                ...
                if s.p95_latency_ms == 0 {
                    ...
                    if duration_ms > s.p95_latency_ms {
                        s.p95_latency_ms =
                            (s.p95_latency_ms as f64 * 0.9 + duration_ms as f64 * 0.1) as u64;
                    } else {
                        s.p95_latency_ms =
                            (s.p95_latency_ms as f64 * 0.99 + duration_ms as f64 * 0.01) as u64;
                    }
                }
            })
        ...
        if let Some(stats) = self.stats.get(&key) {
            if stats.p95_latency_ms > self.sla_latency_p95_ms {
                warn!(
                    provider,
                    model,
                    current_p95 = stats.p95_latency_ms,
                    sla = self.sla_latency_p95_ms,
                    "SLA BREACH"
                );
            }
...
impl SmartRoutingEngine {
    ...
    pub fn select_candidates(
        ...
        ranked.sort_by(|a, b| match strategy {
            ...
            RoutingStrategy::HighestReliability => {
                let err_a = self
                    .qos
                    .get_metrics(&a.0, &a.1)
                    .map(|s| s.error_count as f32 / s.sample_count as f32)
                    .unwrap_or(0.0);
                let err_b = self
                    .qos
                    .get_metrics(&b.0, &b.1)
...
mod tests {
    ...
    fn test_pricing_calculator() {
        ...
        registry.load_from_config(vec![PricingTier {
